...
export const parsePerbicent = (amt?: string | number) =>
  BigNumber.from(amt ? Math.floor(parseFloat(amt.toString())) : 0);
export const fromPerbicent = (amt?: BigNumberish) =>
  amt ? BigNumber.from(amt).toNumber().toString() : "0";
...
export const formattedNum = (
  ...
  if (str.includes(_decimalSeparator)) {
    ...
    return formatNearZero(
      [formattedPre, formattedPost].join(_decimalSeparator)
    );
  }
